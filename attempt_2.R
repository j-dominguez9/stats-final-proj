...
predictions_forward <- predict(forward_model, newdata = test, type = 'response') %>% as.data.frame()
predictions_forward
sum(is.na(predictions_forward$.))
predictions_forward %>% filter(!is.na(.)) %>% summarize(median(.))
predictions_forward <- predictions_forward %>% replace_na(., "11.96347")
here()
forward_sub <- data.frame(Id = test$Id, SalePrice = predictions_forward$.)
head(forward_sub)
